...
class LiuyanAction extends UserAction{
	...
	public function reply(){
		$db = D('reply_info');
		$setInfo=$db->where(array('token'=>$this->token,'infotype'=>'Liuyan'))->find();
		if(IS_POST){
			if($db ->create()){
				if(empty($setInfo)){
					//执行add（）
				...
				if($res){
					$this->success("操作成功",U('Liuyan/index',array('token'=>$this->token)));
					//echo M('distribution_set')->getLastSql();
				}
				else{		
					$this->error("操作失败,请确认输入的值是否有改动",U('Liuyan/index',array('token'=>$this->token)));
					//echo "res:".$res;
					//echo $db->getLastSql();
				}
			}else{
