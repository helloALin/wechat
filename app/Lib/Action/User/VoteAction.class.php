...
class VoteAction extends UserAction
{
	...
	public function totals(){
		...
		$vote_title = M('Vote_item')->where('vid='.$this->_get('id'))->field('item')->order('rank')->select();
        $vote_item = M('Vote_item')->where('vid='.$this->_get('id'))->field('vcount')->order('rank')->select();
		foreach($vote_title as $k=>$val){
			$itemlist[] = "'".$val['item']."'";//拼接选项数组
		}
		$items = implode(',',$itemlist);
		//var_dump($items);
		
		foreach($vote_item as $k=>$val){
			$vlist[] = $val['vcount'];//拼接票数
		}
		$ps = implode(',',$vlist);
		//var_dump($ps);
	...
	public function edit(){
        ...
        if(IS_POST){
            ...
            $items = M('Vote_item')->where('vid='.$id)->order('rank')->select();
            $this->assign('items',$items);
            $this->assign('vo',$vo);
	...
	public function exportSN(){//
        ...
        $letterArr = explode(',', strtoupper('a,b,c,d'));//转换成大写字符串后在','处进行分割
		$arr = array(array('en'=>'item_id','cn'=>'选项ID'),array('en' => 'item', 'cn' => '选项标题'), array('en' => 'wecha_id', 'cn' =>'OpenID'),array('en'=>'touch_time','cn'=>'提交时间'));
        $fieldCount = count($arr);//统计exacle的列数
        $s = 0;
        foreach ($arr as $f) {//循环输出标题行的内容
            if ($s < $fieldCount - 1) {
...
';
		...
		$id = $this -> _get('id');
		$user = new Model();
		$list = $user->table('tp_vote_record r, tp_vote_item i')->where('r.item_id=i.id and r.vid='.$id.'')
					->field('r.item_id, i.item, r.wecha_id,from_unixtime(r.touch_time) as touch_time')
					->order('i.id' )->select();
		//var_dump($list);
		foreach($list as $aa=>$val){
			echo iconv("utf-8", "gbk", $val["item_id"]."\t".$val["item"]
			."\t".$val["wecha_id"]."\t".$val["touch_time"]."\n");
		}
	}
	/*********************导出exacle表************/
	public function exportSN1(){//
        header('Content-Type: text/html; charset=utf-8');
        header('Content-type:application/vnd.ms-execl');
        header('Content-Disposition:filename=huizong.xls');
        $letterArr = explode(',', strtoupper('a,b,c,d'));//转换成大写字符串后在','处进行分割
		$arr = array(array('en'=>'id','cn'=>'选项ID'),array('en' => 'item', 'cn' => '选项标题'), array('en' => 'vcount', 'cn' =>'投票数'));
        $fieldCount = count($arr);//统计exacle的列数
        $s = 0;
        foreach ($arr as $f) {//循环输出标题行的内容
            if ($s < $fieldCount - 1) {
...
';
		...
		//
		$id = $this -> _get('id');
		$list=M('Vote_item')->where(array('vid'=>$id))->field('id,item,vcount')->select();
		//var_dump($list);
		foreach($list as $aa=>$val){
			echo iconv("utf-8", "gbk", $val["id"]."\t".$val["item"]
			."\t".$val["vcount"]."\n");
		}
	}
}
