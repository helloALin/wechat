            ...
            if ($_POST['enddate'] < $_POST['statdate']) {
                ...
                if ($data->create() != false) {
                    if ($id = $data->add()) {
                        ...
                        M('Keyword')->add($data1);
                        $user = M('Users')->where(array('id' => session('uid')))->setInc('activitynum'); //activitynum活动创建数+1
                        $this->_start($id);
