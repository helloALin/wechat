...
class LuckyFruitAction extends LotteryBaseAction{
	public function index(){
		$token		= $this->_get('token');//'qikwvz1409638161';
		$wecha_id	= $this->_get('wecha_id');//'oj1_HjrrXab03ocUO42fqyQtjRAs';
		$id 		= $this->_get('id');
		...
		//$Lottery['endinfo']=$Lottery['endinfo'];
		$data=$Lottery;
		//1.活动过期,显示结束
		//4.显示奖项,说明,时间
		if ($Lottery['enddate'] < time()) {
			 ...
			 $data['endinfo'] = $endinfo;//$Lottery['endinfo'];
			 //var_dump($data['endinfo']);die;
	...
	public function getajax(){	
		...
		$wecha_id	=	$this->_post('wechat_id');//'oj1_HjrrXab03ocUO42fqyQtjRAs';
		$id 		=	$this->_post('id');
		$rid 		= 	$this->_post('rid');
