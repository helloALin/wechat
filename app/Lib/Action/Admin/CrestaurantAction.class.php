...
class CrestaurantAction extends BackAction{
	...
	public function index(){
    	...
    	$rid=($_SESSION['roleid']);
    	//echo $rid;
    	$g_name=M('role')->where("id=$rid")->getField('name');
	...
	function orderInfo(){
		...
		if (IS_POST){
			...
			if (intval($_POST['sent'])){
				//如果发货状态确认发货，则标识为已处理。传‘handled’状态值过去，存入数据库
				$_POST['handled']=1;
			}
			$info123=$product_cart_model->where(array('id'=>$thisOrder['id']))->save(array('sent'=>intval($_POST['sent']),'handled'=>1));
