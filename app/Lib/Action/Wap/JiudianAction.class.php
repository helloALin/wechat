...
class JiudianAction extends BaseAction{
	...
	public function index(){
		...
		if(!empty($info)){
			...
			foreach($info as $k=>&$v){
					$map = array('kind'=> $v["name"],'pid'=>$pid,'token'=> $this->_get('token'),'type'=>array('neq',2));
					$v["shengyu"]=$v["numbers"]-($this->yuyue_order->where($map)->sum(nums));
	...
	public function set(){
		...
		$yy= $this->Yuyue_model->where(array('token'=> $this->_get('token'),'id'=>$pid))->find();
		//print_r($data);print_r($info);die;
