...
class VoteAction extends UserAction{
    ...
    public function totals(){
		...
		$vote_title = M('Vote_item')->where('vid='.$this->_get('id'))->field('item')->select();
        $vote_item = M('Vote_item')->where('vid='.$this->_get('id'))->field('vcount')->select();
...
';
		...
		$id = $this -> _get('id');
		$user = new Model();
		$list = $user->table('tp_vote_record r, tp_vote_item i')->where('r.item_id=i.id')
...
';
		...
		//
		$id = $this -> _get('id');
		$list=M('Vote_item')->field('id,item,vcount')->select();
		//var_dump($list);
		foreach($list as $aa=>$val){
