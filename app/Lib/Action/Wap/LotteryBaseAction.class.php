...
class LotteryBaseAction extends BaseAction
{
    ...
    public function add()
    {
        ...
        if ($_POST['action'] == 'add') {
            ...
            //增加微信回复
				//得到会员卡的ID（只有一种会员卡） by leo
				$cardid=M('Member_card_set')->where(array('token'=>$token))->getField('id');
				//end
                $count=M('member_card_create')->where(array('token'=>$token,'wecha_id'=>$wechaid))->count();
                if(0 == $count){
                    $access_token = getAccessToken($token);
                  	if($access_token["status"]){
		                $l_url = rtrim(C('site_url'), '/') .'/index.php?g=Wap&m=Userinfo&a=index&token='.$token.'&wecha_id='.$wechaid.'&cardid='.$cardid;
		                $picurl = rtrim(C('site_url'), '/') . '/tpl/static/images/member.jpg';
		                $content = '{
		                    "touser": "'.$wechaid.'", 
		                    "msgtype": "news", 
		                    "news": {
		                        "articles": [
		                            {
		                                "title": "会员注册", 
		                                "description": "成为会员，省钱，打折，促销，优先知道，有奖励哦，赶紧加入吧！", 
		                                "url": "'.$l_url.'", 
		                                "picurl": "'.$picurl.'"
		                            }
		                        ]
		                ...
		                $this->api_notice_increment($url,$content);
	               	}
                }
