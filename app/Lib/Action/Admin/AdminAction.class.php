...
class AdminAction extends BackAction{
	...
	public function insert(){
        ...
        if((false == $authInfo)) {
           ...
           // session('userid',$authInfo['id']);  //用户ID
            ...
            session('logintime',date('Y-m-d H:i:s',$authInfo['last_login_time']));
			session('loginip',$authInfo['last_login_ip']);
            //超级管理员识别
    ...
    public function logout() {
        ...
        if(session('?'.C('USER_AUTH_KEY'))) {
            ...
            redirect(U('Admin/Admin/index'));
        }else {
            $this->error('已经登出！',U('Admin/Admin/index'));
        }
    }
}